...
*/
  ...
  function SMIL_generateSlides(data, slide, slideIndex) {
    ...
    if (slide.text) {
      ...
      data.attachments.push({
        id: '<' + id + '>',
        location: id,
        content: new Blob([encoder.encode(slide.text)], {
          type: 'text/plain'
        })
      });
    }
  ...
  function SMIL_generateUniqueLocation(data, location) {
    ...
    while (data.attachments.some(SMIL_uniqueLocationMatches)) {
      result = location.slice(0, index) + '_' + (dupIndex++) + location.slice(index);
    }
    return result;
  }
  ...
  var SMIL = window.SMIL = {
    ...
    parse: function SMIL_parse(message, callback) {
      ...
      function findAttachment(name) {
        ...
        for (; index < length; index++) {
          if (attachments[index].location === name || attachments[index].id === '<' + name + '>') {
      ...
      function SMIL_parseHandleParTag(par, index) {
        ...
        if (textElement) {
          ...
          if (attachment) {
            ...
            // will hold onto
            readTextBlob(attachment.content, function SMIL_parseSMILAttachmentRead(event, text) {
              slide.text = text;
              exitPoint();
            });
          } else {
    ...
    generate: function SMIL_generate(slides) {
      const HEADER = '<head><layout>' + '<root-layout width="320px" height="480px"/>' + '<region id="Image" left="0px" top="0px"' + ' width="320px" height="320px" fit="meet"/>' + '<region id="Text" left="0px" top="320px"' + ' width="320px" height="160px" fit="meet"/>' + '</layout></head>';
      ...
      data.smil = '<smil>' + HEADER + '<body>' + data.parts.join('') + '</body></smil>';
